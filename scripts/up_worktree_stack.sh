...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      ...
      echo "Usage: bash scripts/up_worktree_stack.sh [--skip-runtime-images] [--no-verify]" >&2
      exit 2
      ;;
  esac
done
...
if [[ "$build_runtime_images" == "1" ]]; then
  bash "$repo_root/scripts/build_worktree_runtime_images.sh"
fi
